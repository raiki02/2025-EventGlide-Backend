...
type ActivityDraft struct {
	*gorm.Model
	DraftId        uint   `json:"draft_id" gorm:"not null;type: int;comment:草稿id;column:draft_id"`
	Name           string `json:"name" gorm:"null;type: varchar(255);comment:活动名称;column:name"`
	Details        string `json:"details" gorm:"null;type: text;comment:活动详情;column:details"`
	Type           string `json:"type" gorm:"null;type: varchar(255);comment:活动类型;column:type"`
	Host           string `json:"host" gorm:"null;type: varchar(255);comment:活动主办方;column:host"`
	Location       string `json:"location" gorm:"null;type: varchar(255);comment:活动地点;column:location"`
	IfRegister     bool   `json:"if_register" gorm:"null;type: enum('yes','no');comment:是否需要报名;column:if_register"`
	RegisterMethod string `json:"register_method" gorm:"null;type: varchar(255);comment:报名方式;column:register_method"`
	StartTime      string `json:"start_time" gorm:"null;type: datetime;comment:活动开始时间;column:start_time"`
	EndTime        string `json:"end_time" gorm:"null;type: datetime;comment:活动结束时间;column:end_time"`
	Capacity       int    `json:"capacity" gorm:"null;type: int;comment:活动容量;column:capacity"`
	Visibility     string `json:"visibility" gorm:"null;type: varchar(255);comment:活动可见性;column:visibility"`
}
